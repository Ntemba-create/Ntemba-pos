...
export default function App() {
  ...
  // Custom catalog entry state (supporting Liters, Kg, Meda)
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 font-sans flex flex-col justify-between p-4 sm:p-6">
      ...
      <div className={`w-full mx-auto flex-1 flex flex-col transition-all duration-300 ${tier !== 'tier1' ? 'max-w-6xl' : 'max-w-md'}`}>
        ...
        {/* HEADER */}
        <header className="px-6 py-4 border border-stone-800 bg-stone-900/90 backdrop-blur-md flex flex-col sm:flex-row justify-between items-center gap-4 rounded-2xl mb-4 shadow-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setTier('tier2')} className={`px-3 py-1 rounded-xl text-xs font-bold transition-all cursor-pointer ${tier === 'tier2' ? 'bg-amber-500 text-stone-950' : 'bg-stone-950 text-stone-400 border border-stone-800'}`}>
              Tier 2
            </button>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-6">
          ...
          <button onClick={() => setActiveTab('delivery')} className={`py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${activeTab === 'delivery' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 'bg-stone-900 text-stone-400 border border-stone-800'}`}>
            🚚 Yango
          </button>
          <button onClick={() => setActiveTab('invoices')} className={`py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${activeTab === 'invoices' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 'bg-stone-900 text-stone-400 border border-stone-800'}`}>
            📄 Invoices
          </button>
          <button onClick={() => setActiveTab('market')} className={`py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${activeTab === 'market' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 'bg-stone-900 text-stone-400 border border-stone-800'}`}>
            📢 Ads
          </button>
          <button onClick={() => setActiveTab('council')} className={`py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${activeTab === 'council' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 'bg-stone-900 text-stone-400 border border-stone-800'}`}>
            🏛️ Levies
          </button>
        ...
        <div className="space-y-6 flex-1 w-full">
            ...
            <div className={`grid gap-6 ${tier !== 'tier1' ? 'md:grid-cols-2' : 'grid-cols-1'}`}>
              ...
              <div className="p-4 bg-stone-900/90 border border-stone-800 rounded-3xl space-y-4 flex flex-col justify-between">
                <div className="space-y-4">
                  ...
                  {cart.length === 0 ? (
                    <p className="text-xs text-stone-500 text-center py-6">Cart is empty.</p>
                  ) : (
                ...
                <div className="space-y-3 pt-3 border-t border-stone-800">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button onClick={() => handleCheckout('Cash')} className="py-3 rounded-xl bg-stone-800 hover:bg-stone-700 text-white font-bold text-xs border border-stone-700 cursor-pointer">
                      💵 Cash Tendered
                    </button>
                    <button onClick={() => handleCheckout('Mobile Money')} className="py-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-stone-950 font-bold text-xs shadow-lg shadow-amber-500/20 cursor-pointer">
                      📱 Mobile Money
                    </button>
                  </div>
                </div>
          ...
          {activeTab === 'ledger' && (
            <div className="p-6 bg-stone-900/90 border border-stone-800 rounded-3xl space-y-3 max-w-xl mx-auto w-full">
