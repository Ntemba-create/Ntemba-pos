...
export default function App() {
  ...
  // Auth State
  const [username, setUsername] = useState('TestCashier');
  const [password, setPassword] = useState('••••••••');
  const [selectedTier, setSelectedTier] = useState('Enterprise'); // Default to Enterprise to see all features
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-900 select-none overflow-hidden">
      ...
      {/* 🛠️ DEV DEBUGGER BAR: Lets you instantly jump to any part of the app while coding */}
      <div className="bg-slate-950 border-b border-slate-800 px-3 py-1.5 flex items-center justify-between text-[11px] text-slate-300 overflow-x-auto shrink-0">
        <div className="flex items-center gap-1.5 font-bold text-indigo-400 whitespace-nowrap mr-2">
          <span>Dev Preview:</span>
        </div>
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          <button onClick={() => setView('login')} className={`px-2 py-0.5 rounded ${view === 'login' ? 'bg-indigo-600 text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>Login</button>
          <button onClick={() => setView('signup')} className={`px-2 py-0.5 rounded ${view === 'signup' ? 'bg-indigo-600 text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>Sign Up</button>
          <button onClick={() => setView('forgot')} className={`px-2 py-0.5 rounded ${view === 'forgot' ? 'bg-indigo-600 text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>Forgot Pwd</button>
          <button onClick={() => setView('pos')} className={`px-2 py-0.5 rounded ${view === 'pos' ? 'bg-indigo-600 text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>POS Main</button>
          <span className="text-slate-600">|</span>
          <span className="text-slate-400">Tier:</span>
          <select 
            value={selectedTier} 
            onChange={(e) => setSelectedTier(e.target.value)}
            className="bg-slate-800 text-white border border-slate-700 rounded px-1.5 py-0.5 text-[10px] font-bold"
          >
            <option value="Basic">Basic (3M)</option>
            <option value="Standard">Standard (6M)</option>
            <option value="Enterprise">Enterprise (12M)</option>
          </select>
        </div>
      </div>
      ...
      {view === 'pos' && (
        <div className="flex flex-col flex-1 w-full bg-slate-50 overflow-hidden">
          ...
          <div className="bg-white border-b border-slate-200 px-4 py-2 flex gap-2">
            ...
            <button onClick={() => setActivePosTab('scoring')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${activePosTab === 'scoring' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-700 border border-emerald-200'}`}>🏦 Bank Financial Scoring PDF</button>
          </div>
            ...
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
              <div className="flex-1 p-4 md:p-6 overflow-y-auto flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {filteredProducts(products, activeTab).map(prod => (
                    <div key={prod.id} onClick={() => addItem(prod)} className="bg-white p-4 rounded-2xl border border-slate-200 flex flex-col justify-between h-28 cursor-pointer hover:border-indigo-500">
                      <div>
                        ...
                        {selectedTier !== 'Basic' && <span className="text-[10px] text-slate-400 font-semibold block">Stock: {prod.stock}</span>}
                      </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto bg-slate-50 flex flex-col items-center">
              <div className="w-full max-w-2xl bg-white border border-slate-200 rounded-3xl shadow-xl p-8 flex flex-col gap-6">
                <div className="flex justify-between items-start border-b pb-4">
                  <div>
                    <span className="bg-emerald-100 text-emerald-800 text-[10px] font-extrabold px-2.5 py-1 rounded-full uppercase">Bank-Grade Audit</span>
                    <h2 className="text-xl font-black text-slate-900 mt-2">Trader Financial Health Scorecard</h2>
                    <p className="text-xs text-slate-500">Generated for Commercial Credit Evaluation • {selectedTier} Tier Horizon ({reportMonthsAllowed} Months)</p>
                  </div>
                ...
                <button onClick={() => alert(`Downloaded certified PDF for ${selectedTier} tier!`)} className="w-full py-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-2xl font-black text-sm cursor-pointer">
...
function filteredProducts(products, cat) {
